...
for step in range(13,14): # step = mfcc의 수 13
    ...
    mfcc_feat = librosa.feature.mfcc(y=y, sr=sr, n_fft=512, hop_length=512, n_mfcc=step)
    
    y1, sr1 = librosa.load(path+str(num)+"/MyVoice1.wav")
    mfcc_feat_1 = librosa.feature.mfcc(y=y1, sr=sr1, n_fft=512, hop_length=512, n_mfcc=step)

    y2, sr2 = librosa.load(path+str(num)+"/MyVoice2.wav")
    mfcc_feat_2 = librosa.feature.mfcc(y=y2, sr=sr2, n_fft=512, hop_length=512, n_mfcc=step)

    y3, sr3 = librosa.load(path+str(num)+"/MyVoice3.wav")
    mfcc_feat_3 = librosa.feature.mfcc(y=y3, sr=sr3, n_fft=512, hop_length=512, n_mfcc=step)

    y4, sr4 = librosa.load(path+str(num)+"/MyVoice4.wav")
    mfcc_feat_4 = librosa.feature.mfcc(y=y4, sr=sr4, n_fft=512, hop_length=512, n_mfcc=step)

    y5, sr5 = librosa.load(path+str(num)+"/MyVoice5.wav")
    mfcc_feat_5 = librosa.feature.mfcc(y=y5, sr=sr5, n_fft=512, hop_length=512, n_mfcc=step)

    y6, sr6 = librosa.load(path+str(num)+"/MyVoice6.wav")
    mfcc_feat_6 = librosa.feature.mfcc(y=y6, sr=sr6, n_fft=512, hop_length=512, n_mfcc=step)

    y7, sr7 = librosa.load(path+str(num)+"/MyVoice7.wav")
    mfcc_feat_7 = librosa.feature.mfcc(y=y7, sr=sr7, n_fft=512, hop_length=512, n_mfcc=step)
